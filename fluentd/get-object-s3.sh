...
/bin/cat >/tmp-test/canonical_request.tmp <<EOF
$HTTP_METHOD
$CANONICAL_URI
...
printf %s "$(cat /tmp-test/canonical_request.tmp)" > /tmp-test/canonical_request.tmp

# Generate canonical request hash
CANONICAL_REQUEST_HASH=$(openssl dgst -sha256 /tmp-test/canonical_request.tmp | awk -F ' ' '{print $2}')
...
/bin/cat >/tmp-test/string_to_sign.tmp <<EOF
AWS4-HMAC-SHA256
$DATE_AND_TIME
...
printf %s "$(cat /tmp-test/string_to_sign.tmp)" > /tmp-test/string_to_sign.tmp

# Generate signature
SIGNATURE=$(openssl dgst -sha256 -mac HMAC -macopt hexkey:$HEX_KEY /tmp-test/string_to_sign.tmp | awk -F ' ' '{print $2}')

# Remove temporary files
rm /tmp-test/canonical_request.tmp /tmp-test/string_to_sign.tmp
